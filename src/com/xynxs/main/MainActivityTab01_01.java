...
public class MainActivityTab01_01 implements OnClickListener, OnRefreshListener<ListView> {
	...
	public MainActivityTab01_01(MainActivityTab01 main, View root, Button scopeBtn, TextView topicTv) {
		...
		refresh("", scopeBtn.getText().toString(), topicTv.getText().toString());
	 ...
	 * 刷新当前列表
	 */
	public void refresh(String keyword, String scope, String topic) {
		listView.setRefreshing();
		if (task != null) {
			task.stopTask();
		}
		task = new ListPostTask(main, TAB, topic, scope, 0, COUNT, keyword);
		task.startTask();
	}
